...
The app uses priority-only DND rather than full silence so media playback can continue while interruptions are suppressed.

The app also includes a bottom banner ad integration using Google Mobile Ads test IDs. Replace the sample AdMob app ID and banner ad unit ID before publishing.
...
- `android/app/src/main/kotlin/com/example/dnd_auto/DndManager.kt`: wraps DND policy control and enables priority mode with media allowed.
- `android/app/src/main/kotlin/com/example/dnd_auto/PrefsManager.kt`: stores selected packages, service state, and DND ownership state.
